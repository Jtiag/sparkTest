...
object TypedOperation {
  ...
  def main(args: Array[String]) {
    ...
    // TODO 这个partition的数量不为3
    println(employeeDSCoalesced.rdd.partitions.size)

    //    employeeDSRepartitioned.rdd.mapPartitionsWithIndex {
    //      (partIdx, iter) => {
...
//    employeeDSCoalesced.show()
    ...
    // distinct和dropDuplicates// distinct和dropDuplicates
    ...
    //        employeeDS2.except(employeeDS).show()
...
//    employeeDS.sort($"salary".desc).show()
    ...
    val employeeDSArr = employeeDS.randomSplit(Array(10, 20, 60))
    val employees = employeeDSArr(0).rdd.collect()
    println("+++++++++++++++++++++++++++++"+0);
    for (e <- employees) {
      println(e.name + " " + e.age + " " + e.depId + " " + e.gender + " " + e.salary)
    }
    val employees1 = employeeDSArr(0).rdd.collect()
    println("+++++++++++++++++++++++++++++"+1);
    for (e <- employees1) {
      println(e.name + " " + e.age + " " + e.depId + " " + e.gender + " " + e.salary)
    }
    val employees2 = employeeDSArr(0).rdd.collect()
    println("+++++++++++++++++++++++++++++"+2);
    for (e <- employees2) {
      println(e.name + " " + e.age + " " + e.depId + " " + e.gender + " " + e.salary)
    }
    employeeDS.sample(false, 0.4).show()
  }
}